...
class EncripcionAES:
    ...
    def __init__(self, key):
        ...
        self.key = sha256(key.encode("utf-8")).digest()[:32] #creo una llave con un hash de 256 bits y tomo los primeros 32 (debe ser multiplo de 16)
        ...
        self.unpad = lambda s: s[:-ord(s[len(s)-1:])] # obtenemos el ultimo caracter del padding y con ord lo pasamos a numero, obteniendo el calculo previo para saber cuantos bits quitarle al outpurt

    def leer_archivo(self):
        with open(self.archivo, "r") as documento:
            datos = documento.read() #lee el contenido
            texto = self.pad(datos) #lo padea para rellenar el bloque de bits
            return texto
    ...
    def encripcion(self):
        ...
        cifrar = AES.new(self.key, AES.MODE_OFB, iv) #Creo una instancia AES con la llave y el iv
    ...
    def desencripcion(self):
        ...
        iv = texto_cifrado[:self.tamanio_del_bloque] #obtenemos el iv usado tomando los bits correspondientes al tamanio del bloque del texto usado
